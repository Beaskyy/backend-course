...
const userId = "9419e821-a225-4b75-a77f-1ac30500d4c7";
...
const main = async () => {
  ...
  try {
    ...
    if (!user) {
      console.error(`Error: User with ID ${userId} not found. Please ensure the user exists before seeding movies.`);
      return;
    }

    // Get existing movie titles for this user to ensure idempotency
    ...
    const newMovies = movies.filter((movie) => !existingTitles.has(movie.title));

    if (newMovies.length === 0) {
      console.log("No new movies to seed. All movies already exist in the database.");
    } else {
      console.log(`Adding ${newMovies.length} new movies...`);
      const result = await prisma.movie.createMany({
        data: newMovies,
      });
      console.log(`Successfully created ${result.count} movies.`);
    }
